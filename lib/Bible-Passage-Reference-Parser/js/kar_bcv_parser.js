...
// `translation` is a translation object, but all we care about is the string.
  ...
  bcv_parser.prototype.regexps.escaped_passage = /(?:^|[^\x1f\x1e\dA-Za-zªµºÀ-ÖØ-öø-ɏက-ဪိ-ူဲ-့္-်ွ-ဿၐ-ၕၘ-ၡၥ-ၦၮ-ႂႅ-ႆႍ-ႎႝḀ-ỿⱠ-ⱿꜢ-ꞈꞋ-ꞎꞐ-ꞓꞠ-Ɦꟸ-ꟿꩠ-ꩶꩺ])((?:(?:ch(?:apters?|a?pts?\.?|a?p?s?\.?)?\s*\d+\s*(?:[\u2013\u2014\-]|through|thru|to)\s*\d+\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*)|(?:ch(?:apters?|a?pts?\.?|a?p?s?\.?)?\s*\d+\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*)|(?:\d+(?:th|nd|st)\s*ch(?:apter|a?pt\.?|a?p?\.?)?\s*(?:from|of|in)(?:\s+the\s+book\s+of)?\s*))?\x1f(\d+)(?:\/\d+)?\x1f(?:\/\d+\x1f|[\d\s\xa0.:,;\x1e\x1f&\(\)\uff08\uff09\[\]\/"'\*=~\-\u2013\u2014]|kar(?![a-z])|တၢ်မၤလိ|and|အဆၢ|ff|-|း|[ကခ](?!\w)|$)+)/gi; // Beginning of string or not in the middle of a word or immediately following another book. Only count a book if it's part of a sequence: `Matt5John3` is OK, but not `1Matt5John3`
...
(function(root) {
  ...
  function peg$parse(input, options) {
    ...
    var peg$FAILED = {},
        ...
        peg$c65 = "\u1038",
        peg$c66 = peg$literalExpectation("\u1038", true),
        ...
        peg$c74 = "and",
        peg$c75 = peg$literalExpectation("and", false),
        peg$c76 = function() { return "" },
        peg$c77 = /^[\-\u2013\u2014]/,
        peg$c78 = peg$classExpectation(["-", "\u2013", "\u2014"], false, false),
        peg$c79 = "kar",
        peg$c80 = peg$literalExpectation("kar", false),
